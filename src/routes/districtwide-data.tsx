...
import {
  ...
  Layers, Building2, Calendar, Mail, FileText, CheckCircle2,
} from "lucide-react";

export const Route = createFileRoute("/districtwide-data")({
  component: DistrictwideDataPage,
  head: () => ({
    meta: [
      { title: "Districtwide Data Dashboard — Pre-Bid Intelligence | PIScaleX" },
      ...
      { property: "og:title", content: "Districtwide Data Dashboard — Pre-Bid Intelligence | PIScaleX" },
      ...
      { property: "og:url", content: "/districtwide-data" },
    ],
    links: [{ rel: "canonical", href: "/districtwide-data" }],
  }),
});
    ...
    <header className="fixed top-0 inset-x-0 z-50 border-b border-border/60 backdrop-blur-xl bg-background/70">
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8 text-sm text-muted-foreground">
          ...
          <Link to="/case-studies" className="text-foreground font-medium">Districtwide Data</Link>
          <Link to="/" hash="partner" className="hover:text-foreground transition">Partner</Link>
        </nav>
        <a href="mailto:info@piscalex.us?subject=Strategy Call Request" className="inline-flex items-center gap-1.5 text-sm font-medium px-4 py-2 rounded-lg bg-accent text-accent-foreground hover:opacity-90 transition">
    ...
    <section className="py-24">
      <div className="max-w-5xl mx-auto px-6 text-center">
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-3">
          <a href="mailto:info@piscalex.us?subject=Strategy Call Request" className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-accent text-accent-foreground font-medium hover:opacity-90 transition shadow-lg shadow-accent/20">
            <Calendar className="h-4 w-4" /> Book a Strategy Call
          </a>
          <a href="https://piscalex.us" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 px-6 py-3 rounded-full glass text-foreground font-medium hover:border-accent transition border border-border/60">
...
function Footer() {
  return (
    <footer className="border-t border-border/60 py-10 text-center text-xs text-muted-foreground">
      © {new Date().getFullYear()} PI Solutions and Consulting PVT Limited · Districtwide Data
    </footer>
  );
}
